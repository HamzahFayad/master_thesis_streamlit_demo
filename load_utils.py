...
def build_sidebar(years_df, years_select, country_select):
    ...
    with st.sidebar:
        if years_select and country_select is not None:
            ...
            st.caption(
            f"~ {(years_df['annual_healthcare_expenditure_per_capita'].mean() * (1 + slider_vars['ahec'] / 100)):.2f} int. $"
            )
            st.space()
            ...
            st.caption(
            f"~ {(years_df['gdp_per_capita_worldbank'].mean() * (1 + slider_vars['gdp'] / 100)):.2f} int. $, PPP"
            )  
            st.space()
            ...
            st.caption(
            f"~ {(years_df['nurses_and_midwives_per_1000_people'].mean() * (1 + slider_vars['nm'] / 100)):.2f} per 1000"
            )
            st.space()
            ...
            st.caption(
            f"~ {(years_df['physicians_per_1000_people'].mean() * (1 + slider_vars['phys'] / 100)):.2f} per 1000"
            )
            st.space()
            ...
            st.caption(
            f"~ {( min(100.0, years_df['vaccination_coverage_who_unicef'].mean() + slider_vars['vaccination']) ):.2f} %"
            )
            st.space()
            ...
            min_value=0.0,
            max_value=100 - float(years_df["share_of_population_urban"].max()),
            ...
            st.caption(
            f"~ {( min(100.0, years_df['share_of_population_urban'].mean() + slider_vars['urban']) ):.2f} %"
            )
            st.space()
            ...
            st.caption(
            f"~ {(years_df['prevalence_of_undernourishment'].mean() + slider_vars['undernourishment']):.2f} %"
            )
            st.space()
            ...
            min_value=-float(years_df['share_without_improved_water'].min()) if not years_df['share_without_improved_water'].mean() <= 0.1 else -1.0,
            max_value=0.0,
            value=0.0,
            step=0.5,
            ...
            st.caption(
            f"~ {( years_df['share_without_improved_water'].mean() + slider_vars['water'] ):.2f} %"
            )
            st.space()
            ...
            st.caption(
            f"~ {( years_df['years_of_schooling'].mean() + slider_vars['school']):.1f} school years"
            )
            st.space()
